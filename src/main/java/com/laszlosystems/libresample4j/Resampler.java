...
public class Resampler {
    ...
    private int Yp;
    private double Time;
    private byte[] outByteBuffer = new byte[0];
    private float[] inFloatBuffer = new float[0];
    private float[] outFloatBuffer = new float[0];


    /**
     * Clone an existing resampling session. Faster than creating one from scratch.
     *
     * @param other resampler object
     */
     ...
     * @param factor      bound on resampling factor for this session
     * @throws IllegalArgumentException if factor is not positive
     */
    public Resampler(boolean highQuality, double factor) {
        if (factor <= 0.0) {
        ...
        this.Xoff = (int) (((this.Nmult + 1) / 2.0) * Math.max(1.0, 1.0 / factor) + 10);
        ...
        int YSize = (int) (((double) this.XSize) * factor + 2.0);
    ...
    public Resampler(int inSampleRate, int outSampleRate) {
        this(true, (double) outSampleRate / inSampleRate);
    }

    public Resampler(int inSampleRate, int outSampleRate, boolean highQuality) {
        this(highQuality, (double) outSampleRate / inSampleRate);
    }
     ...
     */
    public boolean process(SampleBuffers buffers, boolean lastBatch) {
        ...
        while (true) {
            ...
            if (factor >= 1) { // SrcUp() is faster if we can use it */
                Nout = lrsSrcUp(this.X, this.Y, factor, Nx, Nwing, LpScl, Imp, ImpD, interpFilt);
            } else {
                Nout = lrsSrcUD(this.X, this.Y, factor, Nx, Nwing, LpScl, Imp, ImpD, interpFilt);
            }
     ...
     * @return true iff resampling is complete (ie. no input samples consumed and no output samples produced)
     */
    public boolean process(final FloatBuffer inputBuffer, final FloatBuffer outputBuffer, boolean lastBatch) {
        ...
        return process(sampleBuffers, lastBatch);
    }
     ...
     */
    public Result process(float[] inBuffer, int inBufferOffset, int inBufferLen, boolean lastBatch,
        ...
        process(inputBuffer, outputBuffer, lastBatch);

        return new Result(inputBuffer.position() - inBufferOffset,
     ...
     */
    public float[] process(float[] inBuffer, boolean lastBatch) {
        if (this.outFloatBuffer.length != (int) (inBuffer.length * this.factor)) {
            this.outFloatBuffer = new float[(int) (inBuffer.length * this.factor)];
        }

        process(inBuffer, 0, inBuffer.length, lastBatch,
                this.outFloatBuffer, 0, this.outFloatBuffer.length);

        return this.outFloatBuffer;
    }

    /**
     * Process a batch of samples. Alternative interface if you prefer to work with arrays.
     *
     * @param inBuffer  array containing input samples in 16 bit little-endian
     * @param lastBatch pass true if this is the last batch of samples
     * @return array with resampled data
     */
    public byte[] process(byte[] inBuffer, boolean lastBatch) {
        if (this.inFloatBuffer.length != inBuffer.length / 2) {
            int inFloatBufferSize = inBuffer.length / 2;
            int outFloatBufferSize = (int) (inFloatBufferSize * this.factor);
            this.inFloatBuffer = new float[inFloatBufferSize];
            this.outFloatBuffer = new float[outFloatBufferSize];
            this.outByteBuffer = new byte[outFloatBufferSize * 2];
        }

        // byte array to float array
        for (int i = 0; i < this.inFloatBuffer.length; i++) {
            this.inFloatBuffer[i] = ((inBuffer[2 * i + 1] << 8) | (inBuffer[2 * i] & 0x00ff)) / 32767.0f;
        }

        process(this.inFloatBuffer, 0, this.inFloatBuffer.length, lastBatch,
                this.outFloatBuffer, 0, this.outFloatBuffer.length);

        // float array to byte array
        for (int i = 0; i < this.outFloatBuffer.length; i++) {
            this.outByteBuffer[2 * i] = (byte) ((int) (this.outFloatBuffer[i] * 32767.0f) & 0xff);
            this.outByteBuffer[2 * i + 1] = (byte) (((int) (this.outFloatBuffer[i] * 32767.0f) >> 8) & 0xff);
        }
    ...
    private int lrsSrcUp(float[] X, float[] Y, double factor, int Nx, int Nwing, float LpScl, float[] Imp,
        ...
        while (CurrentTime < endTime) {
            ...
            v = FilterKit.lrsFilterUp(Imp, ImpD, Nwing, Interp, X, Xp_index++, LeftPhase, -1);
            // Perform right-wing inner product
            v += FilterKit.lrsFilterUp(Imp, ImpD, Nwing, Interp, X, Xp_index, RightPhase, 1);
    ...
    private int lrsSrcUD(float[] X, float[] Y, double factor, int Nx, int Nwing, float LpScl, float[] Imp,
        ...
        while (CurrentTime < endTime) {
            ...
            Xp_index = (int) CurrentTime; // Ptr to current input sample
            // Perform left-wing inner product
            v = FilterKit.lrsFilterUD(Imp, ImpD, Nwing, Interp, X, Xp_index++, LeftPhase, -1, dh);
            // Perform right-wing inner product
            v += FilterKit.lrsFilterUD(Imp, ImpD, Nwing, Interp, X, Xp_index, RightPhase, 1, dh);
